...
def test_kaliski_quantum(v, p):

    if v_cl >= p:
        pytest.skip()

    try:
        expected_inverse = pow(v, -1, p)
    except ValueError:
    ...
    assert quantum_inverse == expected_inverse, f"Quantum inverse {quantum_inverse} did not match expected {expected_inverse} for v={v}, p={p}"
